<?php
//obsahuje základné vlastnosti a metódy pre pohyblivé objekty
class Movable {
    public $x;
    public $y;
    public $width;
...
//Trieda Ball je tiež odvodená z triedy Movable
class Ball extends Movable {
    ...
    public function move() {
        ...
        if (!in_array(Side::Right, $alive_sides)) {
            if ($this->x + GRID > CANVAS_LENGTH - GRID) { 
        ...
        if (!in_array(Side::Bottom, $alive_sides)) {
            if ($this->y + GRID > CANVAS_LENGTH - GRID) { 
        ...
        if ($this->x < GRID && ($this->y < GRID * 2 || $this->y > CANVAS_LENGTH - GRID * 3)) { 
            $this->x = GRID;
            $this->vx *= -1;
            $this->hit_count++;
        }
        else if ($this->x + GRID > CANVAS_LENGTH - GRID && ($this->y < GRID * 2 || $this->y > CANVAS_LENGTH - GRID * 3)) { 
            $this->x = CANVAS_LENGTH - GRID * 2;
            $this->vx *= -1;
            $this->hit_count++;
        }
        else if ($this->y < GRID && ($this->x < GRID * 2 || $this->x > CANVAS_LENGTH - GRID * 3)) { 
            $this->y = GRID;
            $this->vy *= -1;
            $this->hit_count++;
        }
